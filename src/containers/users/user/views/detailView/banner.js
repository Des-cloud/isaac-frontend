...
function Title(props) {
  return (
    <Row>
      <Col>
        <h4 className='my-1 text-center text-sm-left'>{props.children}</h4>
      </Col>
    </Row>
  );
}
  ...
  return (
    <Card className='md-banner rounded'>
          ...
          <Row>
            ...
            <Col className='col-12 col-sm'>
              <FluidContainer>
                ...
                />
                <InfoRow icon='email' content={<Email user={props.user} />} />
                <InfoRow
                  icon='call'
      ...
      <CardFooter className='p-2'>
        <SubtitleBar>
          <SubtitleBarLink path={`${url}`} exact={true} title='About' />
          {props.user.isPhysician ? (
            <>
              <SubtitleBarLink path={`${url}/services`} title='Services' />
              {props.user.username !== props.session.username && (
                <SubtitleBarLink
                  path={`/chats/create/${props.user.username}`}
                  title='Chat'
                />
              )}
            </>
